...
def update_state_geojson(state_count, state_deathcount, date_state_count, date_state_deathcount):
    with open(os.path.join(repo_root, "data/states.geojson")) as f:
        ...
        with open(os.path.join(repo_root, 'data/states_update_processing.geojson'), 'w') as outfile:
...
def update_county_geojson(county_count, county_deathcount, date_county_count, date_county_deathcount):
    ...
    #with open("..data/county_2018.geojson") as f:
        ...
        with open(os.path.join(repo_root, 'data/counties_update_processing.geojson'), 'w') as outfile:
            json.dump(geojson, outfile)
...
try:
    ...
    s3.Object('geoda-covid-atlas2', 'counties_update_processing.geojson').put(Body=open(os.path.join(repo_root, 'data/counties_update.geojson'), 'rb'))
    s3.Object('geoda-covid-atlas2', 'states_update_processing.geojson').put(Body=open(os.path.join(repo_root, 'data/states_update.geojson'), 'rb'))
    print('Write to S3 complete.')
except Exception as e:
    print(e)

os.system('apt install zip')
os.system('rm {}/data/counties_update_processing.geojson.zip data/states_update_processing.geojson.zip'.format(repo_root))
os.system('zip -r {}/data/counties_update_processing.geojson.zip data/counties_update_processing.geojson'.format(repo_root))
os.system('zip -r {}/data/states_update_processing.geojson.zip data/states_update_processing.geojson'.format(repo_root))
