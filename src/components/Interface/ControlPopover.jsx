 ...
 * @category HelperComponents
 * 
 * @param {Object} props.content Content {label: string, items: {value: any, text: string|null, label: string|null}[]}
 * @param {string} props.content.label String label for the control
 * @param {Object[]} props.content.items Array of objects with value, text, and label
 * If using nested, add subItems to each item with the same structure
 * @param {Object|string[]} props.value Value of the control as {value: any, label: any}. 
 * When using `multiple`, an array of value strings.
 * @param {function} props.action Function to call on change
 * @param {boolean} props.multiple Boolean to allow multiple selections. Use SelectMultiControl
 * @param {boolean} props.nested If true, nests rows of checkbox options
 * @param {boolean} props.autocomplete If true, uses autocomplete search instead of select
 * @param {boolean} props.active boolean passed to StyledDropDown for highlighted state
 * overload props passed to MuiSelect
                  ...
                  <Stack direction="row">
                    {item.subItems.map((subItem, subIndex) => (
                      <MenuItem
                        key={subIndex}
                        value={subItem.value}
                        onClick={() => action(subItem.value)}
                      >
                        <Checkbox
                          key={subIndex}
                          checked={value.indexOf(subItem.value) > -1}
                        />
...
const ComboBoxControl = (props) => (
  <SelectControl {...props} autocomplete />
);
...
/**
 * A popover that contains a list of controls for a given component
 ...
 * Typically {type: string, content: string|Object[], value: boolean|Object|string[], action: function}
 * Available types: header, helperText, label, select, switch, slider, comboBox, textInput, selectMulti, selectNestMulti
