...
function VariablePanel() {
  ...
  useLayoutEffect(() => {
    dispatch({
      type: "SET_VARIABLE_MENU_WIDTH",
      payload: variablePanelRef.current.offsetWidth,
    });
  }, []);
      ...
      {panelState.variables && (
        ...
        <Grid xs={12} md={12} lg={6}>
          ...
          <StyledDropDown id="geographySelect" style={{ marginRight: "20px" }}>
            ...
            <Select value={currentPreset.geography} onChange={handleGeography}>
            ...
            <Select value={dataName} onChange={handleDataset}>
              {allDatasets.map((dataset) => (
                <MenuItem
                  ...
                  key={dataset}
                  disabled={
                    variableTree[variableName][currentPreset.geography] ===
                      undefined ||
                    variableTree[variableName][currentPreset.geography].indexOf(
                      dataset
                    ) === -1
