  ...
  *.MuiIconButton-colorSecondary.Mui-checked {
  ...
  &.passive {
    background: ${colors.gray}
      url('${process.env.PUBLIC_URL}/assets/img/animated_cluster.svg');
...
const CsvDownloader = () => {
  ...
  async function GetFiles(fileList) {
    ...
    const dataLinks = await fetch(
      ...
      .then((items) =>
        items
          .filter((d) => fileList[d.name.split('.csv')[0]])
    ...
    const docsLinks = await fetch(
      ...
      .then((r) => r.json())
      .then((items) =>
        items.map((d) => ({ name: d.name, url: d.download_url })),
      );

    // declare promises
    const dataPromises = await dataLinks.map((link) =>
      fetch(link.url).then((r) => r.blob()),
    );
    const docsPromises = await docsLinks.map((link) =>
      fetch(link.url).then((r) => r.blob()),
    );

    // fetch data and docs
    const data = await Promise.all(dataPromises).then((values) =>
      values.map((v, i) => ({
        name: `${dataLinks[i].name.slice(0, -4)}-${new Date()
          .toISOString()
          .slice(0, 10)}.csv`,
        data: v,
      })),
    );
    const docs = await Promise.all(docsPromises).then((values) =>
    ...
    <CsvDownloaderContainer className={isDownloading ? 'passive' : ''}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <p>
            This menu allows you to download bulk CSVs of the data available on
          ...
          <Grid item xs={12} md={4}>
            <FormControl component="fieldset">
              <FormControlLabel
                control={
                  <Checkbox
                    checked={checkboxes[checkboxSet.name]}
                    onChange={() => handleSetChange(i)}
              ...
              <FormGroup>
                {checkboxSet.subset.map((checkboxItem) => (
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={checkboxes[checkboxItem.name]}
