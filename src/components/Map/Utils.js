 ...
 * @param {string} vizType String of the current viz type, 2D, 3D, cartogram, or
 *   dotDensity
 * @param {string} overlays String of overlays to show
 * @param {string} resources String of resources to show
 * @param {string} currData Name of current geojson, used to check if should
 *   show Cartogram text With counties, the text is too much (3000 text labels)
 * @returns {Object[]} A list of deckgl layers to render
 */
function getLayers(layers, vizType, overlays, resources, currData) {
    let LayerArray = LayersByVizType[vizType].map(
    ...
    Object.entries(ResourceLayerMappings).forEach(
        ([key, value]) =>
            resources?.includes(key) && LayerArray.push(layers[value])
    )
    Object.entries(OverLayLayerMappings).forEach(
        ([key, value]) =>
