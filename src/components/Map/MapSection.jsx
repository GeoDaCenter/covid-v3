...
function MapSection({
  ...
  const uiLeftPadding = useSelector(({ ui }) =>
    ui.panelState.variables ? ui.variableMenuWidth : 0
  )
    ...
    if (mapParams.vizType === 'dotDensity') {
      if (!dotDensityData.length) {
        getDotDensityData().then((dotDensityData) =>
          dispatch({ type: 'LOAD_DOT_DENSITY_DATA', payload: dotDensityData })
        )
      }
    }
